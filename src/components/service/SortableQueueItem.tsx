...
export function SortableQueueItem({
  ...
  return (
    <div
      ref={setNodeRef}
      style={style}
      className={`group relative flex flex-col gap-1.5 px-2.5 py-2 rounded-lg border bg-card/50 backdrop-blur-sm transition-all duration-200 ${
        item.enabled
    ...
    >
      <div className="flex items-center gap-2.5">
        ...
        {item.enabled ? (
          <button
            {...attributes}
            {...listeners}
            className="cursor-grab active:cursor-grabbing p-1 rounded hover:bg-muted/80 transition-colors touch-none group-hover:bg-muted/50"
          >
            <GripVertical className="h-3.5 w-3.5 text-muted-foreground" />
          </button>
        ) : (
          <div className="p-1 w-5.5" />
        )}
        ...
        <div className="flex items-center gap-1">
          ...
          <Switch checked={item.enabled} onCheckedChange={() => onToggle(item.id)} className="scale-90" />
      ...
      {item.enabled && definition.options.length > 0 && (
        <div className="ml-11 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 bg-muted/30 rounded-md px-2.5 py-2">
