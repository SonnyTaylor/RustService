...
function FindingsRenderer({ result, definition }: ServiceRendererProps) {
  ...
  const statusBadge = {
    label: isUpToDate ? 'UP TO DATE' : hasInstalled && !hasFailed ? `${data.installed_count} INSTALLED` : hasFailed ? `${data.failed_count} FAILED` : 'CHECKED',
    color: (hasFailed ? 'yellow' : 'green') as 'yellow' | 'green',
  };
